...
export class CreateUserService
	implements
		Service<CreateUserServiceRequest, CreateUserServiceResponse>
{
	...
	async execute(
		...
		try {
            const existingUser =  await this.userRepository.findByEmailOrCpfOrUsername(email, cpf, username)

            if (existingUser.user) {
                return left(new EntityAlreadyExistsError("User with given email, cpf or username already exists."));
            }
